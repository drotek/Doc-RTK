...
const pageFooter = module.exports = {
	...
	hooks: {
		'page:before': function (page) {
			...
			const issues = defaultOption.isShowIssues === true ? '\n{{ "' + defaultOption.repo + '" | listRepo("' + defaultOption.token + '", "' + defaultOption.format + '", ' + defaultOption.utcOffset + ', ' + defaultOption.issueNum + ') }}\n' : '';
	...
	filters: {
		dateFormat: function(d, format, utc) {
			...
			reservedDate.setTime(reservedDate.getTime() + (parseInt(utc) === NaN ? 8 : parseInt(utc)) * 60 * 60 * 1000);
		...
		listRepo: function (d, token, format, utc, issueNum) {
			...
			if (typeof(value) == 'undefined') {
				var url = (token == '') ? 'https://api.github.com/repos/' + d + '/issues?per_page=' + (parseInt(issueNum) === NaN ? 20 : parseInt(issueNum)) : 'https://api.github.com/repos/' + d + '/issues?per_page=8&access_token=' + token;
